...
import { useParams } from "react-router-dom"; // for company-specific routing
...
const OrderTable = () => {
  const { company } = useParams(); // getting company from URL params
  const [orders, setOrders] = useState([]);
  const [pageSize, setPageSize] = useState(10);

  useEffect(() => {
    // Fetching company-specific orders
    if (company && sampleOrders[company]) {
  ...
  const columns = [
    ...
    { field: "amount", headerName: "Amount", width: 150 },
    {
        ...
        renderCell: (params) => {
          ...
          return (
            <div style={{ ...getStatusStyle(status), padding: "5px", borderRadius: "4px", textAlign: "center", width: "100%" }}>
              {status}
            </div>
          );
        },
  ...
  return (
    <div style={{ height: 400, width: "100%" }}>
      <h1>{company ? `${company.charAt(0).toUpperCase() + company.slice(1)} Orders` : "Orders"}</h1>
      <DataGrid
